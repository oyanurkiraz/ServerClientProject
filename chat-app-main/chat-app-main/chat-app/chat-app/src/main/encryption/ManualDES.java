package main.encryption;

import java.util.Base64;
import java.nio.charset.StandardCharsets;

/**
 * DES Manuel Implementasyon (Kütüphanesiz)
 * Feistel Network yapısı ile 16 round DES şifreleme
 * 
 * Bu implementasyon eğitim amaçlıdır ve DES'in çalışma prensiplerini gösterir:
 * - Initial Permutation (IP)
 * - 16 Round Feistel Network
 * - Final Permutation (FP)
 * - S-Box substitution
 * - P-Box permutation
 */
public class ManualDES implements EncryptionAlgorithm {

    private final byte[] key;
    private final long[] subKeys = new long[16];

    // Initial Permutation Table
    private static final int[] IP = {
            58, 50, 42, 34, 26, 18, 10, 2,
            60, 52, 44, 36, 28, 20, 12, 4,
            62, 54, 46, 38, 30, 22, 14, 6,
            64, 56, 48, 40, 32, 24, 16, 8,
            57, 49, 41, 33, 25, 17, 9, 1,
            59, 51, 43, 35, 27, 19, 11, 3,
            61, 53, 45, 37, 29, 21, 13, 5,
            63, 55, 47, 39, 31, 23, 15, 7
    };

    // Final Permutation Table (IP^-1)
    private static final int[] FP = {
            40, 8, 48, 16, 56, 24, 64, 32,
            39, 7, 47, 15, 55, 23, 63, 31,
            38, 6, 46, 14, 54, 22, 62, 30,
            37, 5, 45, 13, 53, 21, 61, 29,
            36, 4, 44, 12, 52, 20, 60, 28,
            35, 3, 43, 11, 51, 19, 59, 27,
            34, 2, 42, 10, 50, 18, 58, 26,
            33, 1, 41, 9, 49, 17, 57, 25
    };

    // Expansion Table (32 bit -> 48 bit)
    private static final int[] E = {
            32, 1, 2, 3, 4, 5,
            4, 5, 6, 7, 8, 9,
            8, 9, 10, 11, 12, 13,
            12, 13, 14, 15, 16, 17,
            16, 17, 18, 19, 20, 21,
            20, 21, 22, 23, 24, 25,
            24, 25, 26, 27, 28, 29,
            28, 29, 30, 31, 32, 1
    };

    // S-Boxes (8 adet, her biri 4x16)
    private static final int[][][] SBOX = {
            // S1
            { { 14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7 },
                    { 0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8 },
                    { 4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0 },
                    { 15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13 } },
            // S2
            { { 15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10 },
                    { 3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5 },
                    { 0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15 },
                    { 13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9 } },
            // S3
            { { 10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8 },
                    { 13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1 },
                    { 13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7 },
                    { 1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12 } },
            // S4
            { { 7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15 },
                    { 13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9 },
                    { 10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4 },
                    { 3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14 } },
            // S5
            { { 2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9 },
                    { 14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6 },
                    { 4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14 },
                    { 11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3 } },
            // S6
            { { 12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11 },
                    { 10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8 },
                    { 9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6 },
                    { 4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13 } },
            // S7
            { { 4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1 },
                    { 13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6 },
                    { 1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2 },
                    { 6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12 } },
            // S8
            { { 13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7 },
                    { 1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2 },
                    { 7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8 },
                    { 2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11 } }
    };

    // P-Box Permutation
    private static final int[] P = {
            16, 7, 20, 21, 29, 12, 28, 17,
            1, 15, 23, 26, 5, 18, 31, 10,
            2, 8, 24, 14, 32, 27, 3, 9,
            19, 13, 30, 6, 22, 11, 4, 25
    };

    // PC-1 (64 bit -> 56 bit)
    private static final int[] PC1 = {
            57, 49, 41, 33, 25, 17, 9,
            1, 58, 50, 42, 34, 26, 18,
            10, 2, 59, 51, 43, 35, 27,
            19, 11, 3, 60, 52, 44, 36,
            63, 55, 47, 39, 31, 23, 15,
            7, 62, 54, 46, 38, 30, 22,
            14, 6, 61, 53, 45, 37, 29,
            21, 13, 5, 28, 20, 12, 4
    };

    // PC-2 (56 bit -> 48 bit)
    private static final int[] PC2 = {
            14, 17, 11, 24, 1, 5,
            3, 28, 15, 6, 21, 10,
            23, 19, 12, 4, 26, 8,
            16, 7, 27, 20, 13, 2,
            41, 52, 31, 37, 47, 55,
            30, 40, 51, 45, 33, 48,
            44, 49, 39, 56, 34, 53,
            46, 42, 50, 36, 29, 32
    };

    // Left shifts for each round
    private static final int[] SHIFTS = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };

    public ManualDES(String key) {
        if (key == null || key.length() < 8) {
            key = padKey(key, 8);
        } else if (key.length() > 8) {
            key = key.substring(0, 8);
        }
        this.key = key.getBytes(StandardCharsets.UTF_8);
        generateSubKeys();
    }

    public ManualDES() {
        this("MDES_KEY");
    }

    private String padKey(String key, int length) {
        if (key == null)
            key = "";
        StringBuilder sb = new StringBuilder(key);
        while (sb.length() < length) {
            sb.append('0');
        }
        return sb.toString();
    }

    private void generateSubKeys() {
        long keyBits = bytesToLong(key);
        long permutedKey = permute(keyBits, PC1, 64, 56);

        int c = (int) (permutedKey >>> 28) & 0x0FFFFFFF;
        int d = (int) permutedKey & 0x0FFFFFFF;

        for (int i = 0; i < 16; i++) {
            c = leftRotate28(c, SHIFTS[i]);
            d = leftRotate28(d, SHIFTS[i]);
            long cd = ((long) c << 28) | d;
            subKeys[i] = permute(cd, PC2, 56, 48);
        }
    }

    private int leftRotate28(int val, int shift) {
        return ((val << shift) | (val >>> (28 - shift))) & 0x0FFFFFFF;
    }

    private long bytesToLong(byte[] bytes) {
        long result = 0;
        for (int i = 0; i < 8; i++) {
            result = (result << 8) | (bytes[i] & 0xFF);
        }
        return result;
    }

    private byte[] longToBytes(long l) {
        byte[] result = new byte[8];
        for (int i = 7; i >= 0; i--) {
            result[i] = (byte) (l & 0xFF);
            l >>>= 8;
        }
        return result;
    }

    private long permute(long input, int[] table, int inputBits, int outputBits) {
        long output = 0;
        for (int i = 0; i < outputBits; i++) {
            int bitPos = table[i];
            long bit = (input >>> (inputBits - bitPos)) & 1;
            output = (output << 1) | bit;
        }
        return output;
    }

    private int feistelFunction(int r, long subKey) {
        // Expansion: 32 bit -> 48 bit
        long expanded = 0;
        for (int i = 0; i < 48; i++) {
            int bitPos = E[i];
            long bit = (r >>> (32 - bitPos)) & 1;
            expanded = (expanded << 1) | bit;
        }

        // XOR with subkey
        long xored = expanded ^ subKey;

        // S-Box substitution: 48 bit -> 32 bit
        int sboxOutput = 0;
        for (int i = 0; i < 8; i++) {
            int block = (int) ((xored >>> (42 - i * 6)) & 0x3F);
            int row = ((block & 0x20) >>> 4) | (block & 1);
            int col = (block >>> 1) & 0x0F;
            int sVal = SBOX[i][row][col];
            sboxOutput = (sboxOutput << 4) | sVal;
        }

        // P-Box permutation
        int pOutput = 0;
        for (int i = 0; i < 32; i++) {
            int bitPos = P[i];
            int bit = (sboxOutput >>> (32 - bitPos)) & 1;
            pOutput = (pOutput << 1) | bit;
        }

        return pOutput;
    }

    private long encryptBlock(long block) {
        // Initial Permutation
        long permuted = permute(block, IP, 64, 64);
        int l = (int) (permuted >>> 32);
        int r = (int) permuted;

        // 16 Rounds
        for (int i = 0; i < 16; i++) {
            int temp = r;
            r = l ^ feistelFunction(r, subKeys[i]);
            l = temp;
        }

        // Swap and Final Permutation
        long preOutput = ((long) r << 32) | (l & 0xFFFFFFFFL);
        return permute(preOutput, FP, 64, 64);
    }

    private long decryptBlock(long block) {
        // Initial Permutation
        long permuted = permute(block, IP, 64, 64);
        int l = (int) (permuted >>> 32);
        int r = (int) permuted;

        // 16 Rounds (reverse order)
        for (int i = 15; i >= 0; i--) {
            int temp = r;
            r = l ^ feistelFunction(r, subKeys[i]);
            l = temp;
        }

        // Swap and Final Permutation
        long preOutput = ((long) r << 32) | (l & 0xFFFFFFFFL);
        return permute(preOutput, FP, 64, 64);
    }

    @Override
    public String encrypt(String plainText) {
        byte[] input = plainText.getBytes(StandardCharsets.UTF_8);
        // PKCS5 Padding
        int padLen = 8 - (input.length % 8);
        byte[] padded = new byte[input.length + padLen];
        System.arraycopy(input, 0, padded, 0, input.length);
        for (int i = input.length; i < padded.length; i++) {
            padded[i] = (byte) padLen;
        }

        byte[] output = new byte[padded.length];
        for (int i = 0; i < padded.length; i += 8) {
            byte[] block = new byte[8];
            System.arraycopy(padded, i, block, 0, 8);
            long encrypted = encryptBlock(bytesToLong(block));
            byte[] encBytes = longToBytes(encrypted);
            System.arraycopy(encBytes, 0, output, i, 8);
        }

        return Base64.getEncoder().encodeToString(output);
    }

    @Override
    public String decrypt(String cipherText) {
        byte[] input = Base64.getDecoder().decode(cipherText);
        byte[] output = new byte[input.length];

        for (int i = 0; i < input.length; i += 8) {
            byte[] block = new byte[8];
            System.arraycopy(input, i, block, 0, 8);
            long decrypted = decryptBlock(bytesToLong(block));
            byte[] decBytes = longToBytes(decrypted);
            System.arraycopy(decBytes, 0, output, i, 8);
        }

        // Remove PKCS5 Padding
        int padLen = output[output.length - 1] & 0xFF;
        if (padLen > 0 && padLen <= 8) {
            byte[] unpadded = new byte[output.length - padLen];
            System.arraycopy(output, 0, unpadded, 0, unpadded.length);
            return new String(unpadded, StandardCharsets.UTF_8);
        }

        return new String(output, StandardCharsets.UTF_8);
    }
}
